...
docker run --rm -ti --user $(id -u):$(id -g) \
    -v /path/to/data:/data ghcr.io/haddocking/powerfit:v5.0.1 \
...
docker run --rm -ti --user $(id -u):$(id -g) \
    -v $PWD:/data ghcr.io/haddocking/powerfit:v5.0.1 \
    /data/ribosome-KsgA.map 13 /data/KsgA.pdb \
    -a 20 -p 2 -d /data/run-KsgA-docker
```

To run on NVIDIA GPU using [NVIDIA container toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/index.html) use the CUDA image
```shell
docker run --rm -ti \
    --runtime=nvidia --gpus all \
    -v $PWD:/data ghcr.io/haddocking/powerfit-cuda13:v5.0.1 \
    /data/ribosome-KsgA.map 13 /data/KsgA.pdb \
    -a 20 -d /data/run-KsgA-docker-nv --gpu
```
Use `ghcr.io/haddocking/powerfit-cuda12:v5.0.1` if you have CUDA version 12 (see `nvidia-smi` for version).
...
sudo docker run --rm -ti \
    ...
    -v $PWD:/data ghcr.io/haddocking/powerfit-rocm:v5.0.1 \
    /data/ribosome-KsgA.map 13 /data/KsgA.pdb \
