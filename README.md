...
docker run --rm -ti --user $(id -u):$(id -g) \
    -v /path/to/data:/data ghcr.io/haddocking/powerfit:v3.0.3 \
...
```shell
docker build -t ghcr.io/haddocking/powerfit:v3.0.3 .
```
The Docker container, that works for AMD gpus, can be build with

```shell
docker build -t ghcr.io/haddocking/powerfit-rocm:v3.0.3 -f Dockerfile.rocm .
```
